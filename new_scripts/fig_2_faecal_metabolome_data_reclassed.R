fecmet <- read.delim("raw/urine_reclustered/faecal_metabs_classified.tsv", sep = "\t", header = FALSE)
...
fecmet_info <- fecmet %>% 
  ...
  data.frame() %>% 
  dplyr::filter(!V3 %in% c("SAMPLE","visit","ID")) %>% 
  dplyr::select(c(!V3)) %>% 
...
fecmet <- fecmet %>% 
  ...
  mutate(visit = case_when(V1 == visit ~ "visit",.default = visit), 
         ID    = case_when(is.na(ID)~"ID",.default = ID)) %>% 
  dplyr::select(!c(V1)) %>% 
...
fecmet <- fecmet %>% 
    left_join(., fecmet_info, by = c("name" = "V2")) %>% 
    mutate(value = as.numeric(value)) %>% 
    mutate(name = str_remove(name, " $")) %>% 
    filter(ID != "ID") %>% 
  
    # group_by(ID, V1, visit) %>% 
    # mutate(value = sum(value)) %>% 
    # ungroup() %>% 
    # 
    dplyr::select(ID, visit, name, value) %>%
...
fecmet = fecmet[rowSums(fecmet[,-c(1:2)])!=0,]


fecmet[,-c(1:2)] <-  fecmet[,-c(1:2)] %>% t() %>% Tjazi::clr_c() %>% t()

fecmet_df_long <-  fecmet %>% 
  left_join(., metadata %>% 
...
fecmet_df_long %>% 
  ...
  ungroup() %>% 
  arrange(desc(abs(delta))) %>% .$name -> fecmet_order
...
fecmet_df_long %>% 
  ...
  ungroup() %>% 
  filter(name %in% fecmet_order) %>% 
  mutate(name  = factor(name, levels = fecmet_order)) -> fecmet_df_long


CAF_DECAF_order <-  c(fecmet_df_long %>% 
                        filter(Visit == "Post-\nreintroduction") %>% 
...
plot_fecmet_reclass_NCD <- fecmet_df_long %>% 
...
plot_fecmet_reclass_CD <-  fecmet_df_long %>% 
  filter(Coffee_Type != "NCD") %>% 
...
#(plot_fecmet_NCD + plot_fecmet_CD) + plot_layout(guides = 'collect')
