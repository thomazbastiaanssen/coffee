...
df_cyto_long <- rbind(stim_df, unstim_df) %>% 
...
df_cyto_long %>% 
  ...
  ungroup() %>% 
  arrange(desc(abs(delta))) %>% .$name -> cyto_order



CAF_DECAF_order <-  c(df_cyto_long %>% 
                        filter(Visit == "Post-\nreintroduction") %>% 
...
plot_cyt_stim_NCD <- df_cyto_long %>% 
...
plot_cyt_unstim_NCD <- df_cyto_long %>% 
  filter(Coffee_Type =="NCD") %>% 
  mutate(lab = "NCD") %>% 
  filter(type == "unstim") %>% 
  filter(!is.na(value)) %>% 
...
plot_cyt_unstim_CD <- df_cyto_long %>% 
  filter(Coffee_Type != "NCD") %>% 
  filter(type == "unstim") %>% 
  ...
  scale_y_discrete(expand = expansion(0,0), position = "right"
                   #   , labels = c(rep("",7), "DECAF",rep("",15),  "CAF", rep("",7))
  ) +
  scale_x_discrete(expand = expansion(mult = c(0))) +
  ggh4x::facet_nested(name * caffeine ~ mdlab, scales = "free", 
  ...
  theme(#strip.text.y = element_blank(), 
    ...
    #axis.text.x = element_text(angle = 330, hjust = 0)#,   panel.spacing.x =unit(0, "lines")
    axis.text.x = element_blank(), axis.ticks.x = element_blank(), 
    panel.spacing.x = unit(0, "lines"), strip.clip = "off"
