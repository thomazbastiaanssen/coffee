...
# fecmet_df_long %>% group_by(name, visit, mdlab) %>% 
#   
...
MBfecmet_R_marg <- matrix(NA, nrow = ncol(fecMX), 
                      ncol = ncol(MB), 
                      dimnames = list(colnames(fecMX), colnames(MB)))


MBfecmet_P_marg <- MBfecmet_R_marg


umcog_R_marg <- matrix(NA, nrow = ncol(fecMX), 
                       ncol = ncol(cog), 
                       dimnames = list(colnames(fecMX), colnames(cog)))
...
for(fm in 1:ncol(fecMX)){
  ...
  for(mb in 1:ncol(MB)){
    ...
    MBfecmet_P_marg[fm,mb] = anova(mod.0, mod.1)["mod.1", "Pr(>Chisq)"]
    MBfecmet_R_marg[fm,mb] = modelsummary::get_gof(mod.1)[["r2.marginal"]]
  }
}


for(fm in 1:ncol(fecMX)){
  df_totumcog$y = unlist(fecMX[,fm])
  ...
  for(cg in 1:ncol(cog)){
    ...
    if(identical(unname(is.na(df_totumcog$x)), unname(is.na(df_totumcog$y)))){
      ...
      umcog_P_marg[fm,cg] = anova(mod.0, mod.1)["mod.1", "Pr(>Chisq)"]
      umcog_R_marg[fm,cg] = modelsummary::get_gof(mod.1)[["r2.marginal"]]}  
    ...
    if(!identical(unname(is.na(df_totumcog$x)), unname(is.na(df_totumcog$y)))){
      ...
      umcog_P_marg[fm,cg] = anova(mod.0.temp, mod.1)["mod.1", "Pr(>Chisq)"]
      umcog_R_marg[fm,cg] = modelsummary::get_gof(mod.1)[["r2.marginal"]]}  
  }
}
...
fig_int_mb_fecmet_cog <- rbind(umcog_P_marg %>% 
                   as.data.frame() %>% 
                   rownames_to_column("fecmetabolites") %>% 
                   pivot_longer(!"fecmetabolites", names_to = "non_mb", values_to = "p.val") %>% 
                   left_join(., umcog_R_marg %>% as.data.frame() %>% 
                               rownames_to_column("fecmetabolites") %>% 
                               pivot_longer(!"fecmetabolites", names_to = "non_mb", values_to = "Rsq"), 
                             by = c("fecmetabolites", "non_mb")) %>% 
                   
                   mutate(p.adj = p.adjust(p.val, method = "bonferroni")) %>% 
                   dplyr::select(c("fecmetabolites", "non_mb", "p.val", "Rsq", "p.adj")) %>% 
                 ...
                 MBfecmet_P_marg %>% 
                   ...
                   rownames_to_column("fecmetabolites") %>% 
                   pivot_longer(!"fecmetabolites", names_to = "non_mb", values_to = "p.val") %>% 
                   left_join(., MBfecmet_R_marg %>% as.data.frame() %>% 
                               rownames_to_column("fecmetabolites") %>% 
                               pivot_longer(!"fecmetabolites", names_to = "non_mb", values_to = "Rsq"), 
                             by = c("fecmetabolites", "non_mb")) %>% 
  ...
  arrange(type, non_mb, fecmetabolites) %>%
  mutate(sig = case_when((Rsq > 0.2  & p.adj < 0.01) ~ "sig",
                         (Rsq > 0.1  & p.adj < 0.01) ~ "weaksig",
                         
                         .default = "nonsig")) %>% 
  # select(fecmetabolites, type, non_mb, p.val,p.adj, Rsq) %>% 
  # 
  # ggplot() + 
  # aes(x = p.adj, y = Rsq) +
  # geom_point() +
  # facet_wrap(~fecmetabolites, scales = "free_x")
  # 
  
  
  as_tbl_graph() %>% #.[1] %>% View
  mutate(label = name, 
         xpos = case_when(name %in% colnames(fecMX)  ~ 2, 
  ...
  geom_diagonal_wide(data = . %>% filter(sig != "nonsig"),aes(x = x_val, y = y_val, group = id, fill = metab_name, alpha = sig), colour = "black") +
  #urine metabolome
  geom_rect(data = . %>% pluck("metab_name") %>% levels() %>% tibble() %>% dplyr::rename("metab_name" = ".") %>% 
              mutate(
                xmin = 1.75, 
                xmax = 2.25,
                ymin = (1:4)/4*1.75,# + (1.75 * 0.1) - 0.075,
                ymax = (1:4)/4*1.75 + 0.1),
  ...
  geom_label(data = . %>% filter(type == "microbes") %>% 
               filter(sig != "nonsig") %>% dplyr::select(nonmetab_name) %>% distinct() %>% 
